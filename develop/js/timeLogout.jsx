define( function( require, exports, module ) {
	 ...
	 */
	var StateAndTime = React.createClass( {
		...
		componentDidMount: function() {
	  	...
	  	render: function() {
	  		...
	  		noticeList = this.props.state.storageCondition.map(function(notice, index) {
	  			return <li key={index}>{index + 1 + '. ' + notice}</li>;
	  		})
			...
			switch( new Date( startTime ).getDay() ){
	  		...
	  		return  <div className={ 'm-state '+stateClass}>
	  					<div className="notice-wrap">
							<ul className="notice-par" style={{'animationDuration': noticeList.length * 3 + 's'}}>
								{noticeList}
							</ul>
						</div>
	  					<span className='dot'></span>
	  					<span className='des'>{stateDes}</span>
	  					<span className='time'>{COMMON_FUN.formateDate(startTime,'YYYY-MM-DD hh:mm:ss')+' '+week}</span>
	  				</div>;
		}
	} );
